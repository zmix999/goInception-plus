...
import (
	...
	txninfo "gitee.com/zhoujin826/goInception-plus/session/txninfo"
...
func (s *inspectionSuite) TestInspectionTables(c *C) {
	...
	fpName := "gitee.com/zhoujin826/goInception-plus/infoschema/mockClusterInfo"
...
// https://gitee.com/zhoujin826/goInception-plus/issues/25467.
...
func (s *testInfoschemaClusterTableSuite) TestTiDBClusterInfo(c *C) {
	...
	c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/infoschema/mockStoreTombstone", `return(true)`), IsNil)
	tk.MustQuery("select type, instance, start_time from information_schema.cluster_info where type = 'tikv'").Check(testkit.Rows())
	c.Assert(failpoint.Disable("gitee.com/zhoujin826/goInception-plus/infoschema/mockStoreTombstone"), IsNil)
