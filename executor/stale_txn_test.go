...
func (s *testStaleTxnSerialSuite) TestSelectAsOf(c *C) {
	...
	for _, testcase := range testcases1 {
		...
		c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/executor/assertStaleTSO", fmt.Sprintf(`return(%d)`, testcase.expectPhysicalTS)), IsNil)
		...
		c.Assert(failpoint.Disable("gitee.com/zhoujin826/goInception-plus/executor/assertStaleTSO"), IsNil)
		if len(testcase.setTxnSQL) > 0 {
	...
	for _, testcase := range testcases2 {
		...
		if testcase.preSec > 0 {
			c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/expression/injectNow", fmt.Sprintf(`return(%d)`, now.Unix())), IsNil)
			c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/executor/assertStaleTSO", fmt.Sprintf(`return(%d)`, now.Unix()-testcase.preSec)), IsNil)
		}
		rs, err := tk.Exec(testcase.sql)
		if len(testcase.errorStr) != 0 {
			c.Assert(err, ErrorMatches, testcase.errorStr)
		...
		if testcase.preSec > 0 {
			c.Assert(failpoint.Disable("gitee.com/zhoujin826/goInception-plus/expression/injectNow"), IsNil)
			c.Assert(failpoint.Disable("gitee.com/zhoujin826/goInception-plus/executor/assertStaleTSO"), IsNil)
		}
	}
}
...
func (s *testStaleTxnSerialSuite) TestStaleReadKVRequest(c *C) {
	...
	testcases := []struct {
	...
	}{
		{
			...
			sql:    "select * from t",
			assert: "gitee.com/zhoujin826/goInception-plus/distsql/assertRequestBuilderReplicaOption",
		},
		{
			name:   "point get read",
			sql:    "select * from t where id = 1",
			assert: "gitee.com/zhoujin826/goInception-plus/executor/assertPointReplicaOption",
		},
		{
			name:   "batch point get read",
			sql:    "select * from t where id in (1,2,3)",
			assert: "gitee.com/zhoujin826/goInception-plus/executor/assertBatchPointReplicaOption",
		},
	}
...
func (s *testStaleTxnSerialSuite) TestTimeBoundedStalenessTxn(c *C) {
	...
	for _, testcase := range testcases {
		...
		c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/expression/injectSafeTS",
			fmt.Sprintf("return(%v)", testcase.injectSafeTS)), IsNil)
		tk.MustExec(testcase.sql)
	...
	failpoint.Disable("gitee.com/zhoujin826/goInception-plus/expression/injectSafeTS")
...
func (s *testStaleTxnSerialSuite) TestStaleReadPrepare(c *C) {
	...
	failpoint.Enable("gitee.com/zhoujin826/goInception-plus/executor/assertExecutePrepareStatementStalenessOption",
	...
	tk.MustExec("execute p2")
	failpoint.Disable("gitee.com/zhoujin826/goInception-plus/executor/assertExecutePrepareStatementStalenessOption")
...
func (s *testStaleTxnSerialSuite) TestStmtCtxStaleFlag(c *C) {
	...
	for _, testcase := range testcases {
		failpoint.Enable("gitee.com/zhoujin826/goInception-plus/exector/assertStmtCtxIsStaleness",
			fmt.Sprintf("return(%v)", testcase.hasStaleFlag))
		tk.MustExec(testcase.sql)
		failpoint.Disable("gitee.com/zhoujin826/goInception-plus/exector/assertStmtCtxIsStaleness")
...
func (s *testStaleTxnSerialSuite) TestStaleSessionQuery(c *C) {
	...
	c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/expression/injectNow", fmt.Sprintf(`return(%d)`, now.Unix())), IsNil)
	c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/executor/assertStaleTSO", fmt.Sprintf(`return(%d)`, now.Unix()-1)), IsNil)
...
func (s *testStaleTxnSerialSuite) TestStaleReadCompatibility(c *C) {
	...
	tk.MustExec("set @@tidb_read_staleness='-1'")
	c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/expression/injectNow", fmt.Sprintf(`return(%d)`, t1.Unix())), IsNil)
