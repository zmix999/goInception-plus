...
goInception-plus itself is based on TiDB, so it has a complete user management module. For ease of use, this function is turned off by default.
...
If you forget the password, you can restart it by skipping authentication. After changing the password, turn on authentication and restart goInception-plus (this method is similar to MySQL).
