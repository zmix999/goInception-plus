...
func TestStoreErr(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("gitee.com/zhoujin826/goInception-plus/infoschema/mockTiFlashStoreCount", `return(true)`))
	defer func() {
		require.NoError(t, failpoint.Disable("gitee.com/zhoujin826/goInception-plus/infoschema/mockTiFlashStoreCount"))
	}()
	...
	require.NoError(t, failpoint.Enable("gitee.com/zhoujin826/goInception-plus/store/mockstore/unistore/BatchCopCancelled", "1*return(true)"))

	err = tk.QueryToErr("select count(*) from t")
	require.Equal(t, context.Canceled, errors.Cause(err))

	require.NoError(t, failpoint.Enable("gitee.com/zhoujin826/goInception-plus/store/mockstore/unistore/BatchCopRpcErrtiflash0", "1*return(\"tiflash0\")"))
...
func TestStoreSwitchPeer(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("gitee.com/zhoujin826/goInception-plus/store/mockstore/unistore/BatchCopRpcErrtiflash1", "return(\"tiflash1\")"))
	err = tk.QueryToErr("select count(*) from t")
	require.Error(t, err)
}
