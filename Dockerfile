...
COPY bin/goInception-plus /goInception-plus
...
COPY --from=builder /goInception-plus /goInception-plus
...
ENTRYPOINT ["/usr/local/bin/dumb-init", "/goInception-plus","--config=/etc/config.toml"]
