...
func (s *testColumnChangeSuite) TestFailBeforeDecodeArgs(c *C) {
	...
	tc.onJobRunBefore = func(job *model.Job) {
		...
		if job.SchemaState == model.StateWriteOnly {
			...
			if first {
				c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/ddl/errorBeforeDecodeArgs", `return(true)`), IsNil)
				first = false
			} else {
				c.Assert(failpoint.Disable("gitee.com/zhoujin826/goInception-plus/ddl/errorBeforeDecodeArgs"), IsNil)
			}
		}
	}
	d.SetHook(tc)
