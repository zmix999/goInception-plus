...
func (s *testFilterSuite) TestFilter(c *C) {
	...
	logger, buffer = log.MakeTestLogger(zap.WrapCore(func(c zapcore.Core) zapcore.Core {
		return log.NewFilterCore(c, "gitee.com/zhoujin826/goInception-plus/br/")
	}), zap.AddCaller())
	logger.Warn("the message", zap.Int("number", 123456), zap.Ints("array", []int{7, 8, 9}))
	...
	logger, buffer = log.MakeTestLogger(zap.WrapCore(func(c zapcore.Core) zapcore.Core {
		return log.NewFilterCore(c, "gitee.com/zhoujin826/goInception-plus/br/").With([]zap.Field{zap.String("a", "b")})
	}), zap.AddCaller())
	logger.Warn("the message", zap.Int("number", 123456), zap.Ints("array", []int{7, 8, 9}))
	c.Assert(buffer.Stripped(), HasLen, 0)
...
//
// Run `go test gitee.com/zhoujin826/goInception-plus/br/pkg/lightning/log -check.b -test.v` to get benchmark result.
func (s *testFilterSuite) BenchmarkFilterStringsContains(c *C) {
	c.ResetTimer()

	inputs := []string{
		"gitee.com/zhoujin826/goInception-plus/some/package/path",
		"github.com/tikv/pd/some/package/path",
		"gitee.com/zhoujin826/goInception-plus/br/some/package/path",
	}
	filters := []string{"gitee.com/zhoujin826/goInception-plus/", "github.com/tikv/pd/"}
	for i := 0; i < c.N; i++ {
		for i := range inputs {
			for j := range filters {
