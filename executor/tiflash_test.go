...
// to fix https://gitee.com/zhoujin826/goInception-plus/issues/27952
...
func (s *tiflashTestSuite) TestMppExecution(c *C) {
	...
	failpoint.Enable("gitee.com/zhoujin826/goInception-plus/executor/checkTotalMPPTasks", `return(3)`)
	...
	failpoint.Disable("gitee.com/zhoujin826/goInception-plus/executor/checkTotalMPPTasks")
...
func (s *tiflashTestSuite) TestPartitionTable(c *C) {
	...
	tk.MustExec("insert into t values(3,0)")
	tk.MustExec("insert into t values(4,0)")
	failpoint.Enable("gitee.com/zhoujin826/goInception-plus/executor/checkUseMPP", `return(true)`)
	tk.MustExec("set @@session.tidb_isolation_read_engines=\"tiflash\"")
	tk.MustExec("set @@session.tidb_allow_mpp=ON")
	// mock executor does not support use outer table as build side for outer join, so need to
	// force the inner table as build side
	tk.MustExec("set tidb_opt_mpp_outer_join_fixed_build_side=1")
	failpoint.Enable("gitee.com/zhoujin826/goInception-plus/executor/checkTotalMPPTasks", `return(4)`)
	tk.MustQuery("select count(*) from t").Check(testkit.Rows("4"))
	failpoint.Disable("gitee.com/zhoujin826/goInception-plus/executor/checkTotalMPPTasks")
	tk.MustExec("set @@session.tidb_partition_prune_mode='static-only'")
	failpoint.Enable("gitee.com/zhoujin826/goInception-plus/executor/checkUseMPP", `return(false)`)
	tk.MustQuery("select count(*) from t").Check(testkit.Rows("4"))
	...
	failpoint.Enable("gitee.com/zhoujin826/goInception-plus/executor/checkTotalMPPTasks", `return(8)`)
	...
	failpoint.Enable("gitee.com/zhoujin826/goInception-plus/executor/checkTotalMPPTasks", `return(9)`)
	...
	failpoint.Enable("gitee.com/zhoujin826/goInception-plus/executor/checkTotalMPPTasks", `return(7)`)
	tk.MustQuery("select count(*) from t, t3 where t3.a = t.a and t3.b <= 4").Check(testkit.Rows("3"))
	failpoint.Disable("gitee.com/zhoujin826/goInception-plus/executor/checkTotalMPPTasks")
	failpoint.Enable("gitee.com/zhoujin826/goInception-plus/executor/checkTotalMPPTasks", `return(5)`)
	tk.MustQuery("select count(*) from t, t3 where t3.a = t.a and t3.b > 10").Check(testkit.Rows("0"))
	failpoint.Disable("gitee.com/zhoujin826/goInception-plus/executor/checkTotalMPPTasks")
	failpoint.Disable("gitee.com/zhoujin826/goInception-plus/executor/checkUseMPP")
}
...
func (s *tiflashTestSuite) TestDispatchTaskRetry(c *C) {
	...
	c.Assert(err, IsNil)
	tk.MustExec("set @@session.tidb_enforce_mpp=ON")
	c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/store/mockstore/unistore/mppDispatchTimeout", "3*return(true)"), IsNil)
	tk.MustQuery("select count(*) from t").Check(testkit.Rows("4"))
	c.Assert(failpoint.Disable("gitee.com/zhoujin826/goInception-plus/store/mockstore/unistore/mppDispatchTimeout"), IsNil)
}

func (s *tiflashTestSuite) TestCancelMppTasks(c *C) {
	testleak.BeforeTest()
	defer testleak.AfterTest(c)()
	var hang = "gitee.com/zhoujin826/goInception-plus/store/mockstore/unistore/mppRecvHang"
	tk := testkit.NewTestKit(c, s.store)
	tk.MustExec("use test")
	tk.MustExec("drop table if exists t")
	tk.MustExec("create table t(a int not null primary key, b int not null)")
...
func (s *tiflashTestSuite) TestMppGoroutinesExitFromErrors(c *C) {
	...
	var mppNonRootTaskError = "gitee.com/zhoujin826/goInception-plus/store/copr/mppNonRootTaskError"
...
func (s *tiflashTestSuite) TestMppUnionAll(c *C) {
	...
	failpoint.Enable("gitee.com/zhoujin826/goInception-plus/executor/checkTotalMPPTasks", `return(6)`)
	tk.MustQuery("select count(*) from (select * from x1 union all select * from x2 union all select * from x3) x join (select * from x1 union all select * from x2 union all select * from x3) y on x.a = y.b").Check(testkit.Rows("29"))
	failpoint.Disable("gitee.com/zhoujin826/goInception-plus/executor/checkTotalMPPTasks")
