...
func (s *session) pgRaw(sqlStr string) (rows *sql.Rows, err error) {
	// 连接断开无效时,自动重试
...
func (s *session) pgExecSQL(sqlStr string, retry bool) (res sql.Result, err error) {
	log.Debug("exec")
	// 连接断开无效时,自动重试
	for i := 0; i < maxBadConnRetries; i++ {
...
func (s *session) pgExecDDL(sqlStr string, retry bool) (res sql.Result, err error) {
	log.Debug("execDDL")
	// 连接断开无效时,自动重试
	for i := 0; i < maxBadConnRetries; i++ {
...
// Raw 执行sql语句,连接失败时自动重连,自动重置当前数据库
func (s *session) pgRawScan(sqlStr string, dest interface{}) (err error) {
	// 连接断开无效时,自动重试
	for i := 0; i < maxBadConnRetries; i++ {
...
// Raw 执行sql语句,连接失败时自动重连,自动重置当前数据库
func (s *session) PgRawDB(dest interface{}, sqlStr string, values ...interface{}) (err error) {
	// 连接断开无效时,自动重试
	for i := 0; i < maxBadConnRetries; i++ {
...
func (s *session) pgInitConnection() (err error) {
