...
func (ts *HTTPHandlerTestSuite) TestRegionsFromMeta(c *C) {
	...
	// test no panic
	c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/server/errGetRegionByIDEmpty", `return(true)`), IsNil)
	resp1, err := ts.fetchStatus("/regions/meta")
	c.Assert(err, IsNil)
	defer resp1.Body.Close()
	c.Assert(failpoint.Disable("gitee.com/zhoujin826/goInception-plus/server/errGetRegionByIDEmpty"), IsNil)
}
...
func (ts *HTTPHandlerTestSuite) TestFailpointHandler(c *C) {
	...
	c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/server/enableTestAPI", "return"), IsNil)
	ts.startServer(c)
	resp, err = ts.fetchStatus("/fail/")
	c.Assert(err, IsNil)
	c.Assert(resp.StatusCode, Equals, http.StatusOK)
	...
	c.Assert(err, IsNil)
	c.Assert(strings.Contains(string(b), "gitee.com/zhoujin826/goInception-plus/server/enableTestAPI=return"), IsTrue)
	c.Assert(resp.Body.Close(), IsNil)
}
