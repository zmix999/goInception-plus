...
func (s *testSerialSuite1) testSortInDisk(c *C, removeDir bool) {
	...
	c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/executor/testSortedRowContainerSpill", "return(true)"), IsNil)
	defer func() {
		c.Assert(failpoint.Disable("gitee.com/zhoujin826/goInception-plus/executor/testSortedRowContainerSpill"), IsNil)
	}()
...
func (s *testSerialSuite1) TestIssue16696(c *C) {
	...
	c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/executor/testRowContainerSpill", "return(true)"), IsNil)
	defer func() { c.Assert(failpoint.Disable("gitee.com/zhoujin826/goInception-plus/executor/testRowContainerSpill"), IsNil) }()
	tk := testkit.NewTestKit(c, s.store)
	tk.MustExec("use test")
	tk.MustExec("drop table if exists t")
