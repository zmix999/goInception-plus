...
func (s *testSerialSuite1) TestShuffleMergeJoinInDisk(c *C) {
	...
	c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/executor/testMergeJoinRowContainerSpill", "return(true)"), IsNil)
	defer func() {
		c.Assert(failpoint.Disable("gitee.com/zhoujin826/goInception-plus/executor/testMergeJoinRowContainerSpill"), IsNil)
	}()
...
func (s *testSuite2) TestMergeJoin(c *C) {
	...
	// Test TIDB_SMJ for join with order by desc, see https://gitee.com/zhoujin826/goInception-plus/issues/14483
	tk.MustExec("drop table if exists t")
	tk.MustExec("drop table if exists t1")
