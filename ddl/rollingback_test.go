...
func (s *testRollingBackSuite) TestCancelAddIndexJobError(c *C) {
	...
	c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/ddl/mockConvertAddIdxJob2RollbackJobError", `return(true)`), IsNil)
	defer func() {
		c.Assert(failpoint.Disable("gitee.com/zhoujin826/goInception-plus/ddl/mockConvertAddIdxJob2RollbackJobError"), IsNil)
	}()
