...
func (s *testSuite10) TestFailedAnalyzeRequest(c *C) {
	...
	tk.MustExec("create table t(a int primary key, b int, index index_b(b))")
	tk.MustExec("set @@tidb_analyze_version = 1")
	c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/executor/buildStatsFromResult", `return(true)`), IsNil)
	_, err := tk.Exec("analyze table t")
	c.Assert(err.Error(), Equals, "mock buildStatsFromResult error")
	c.Assert(failpoint.Disable("gitee.com/zhoujin826/goInception-plus/executor/buildStatsFromResult"), IsNil)
}
...
func (s *testSerialSuite2) TestAnalyzeSamplingWorkPanic(c *C) {
	...
	c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/executor/mockAnalyzeSamplingBuildWorkerPanic", "return(1)"), IsNil)
	err := tk.ExecToErr("analyze table t")
	c.Assert(err, NotNil)
	c.Assert(failpoint.Disable("gitee.com/zhoujin826/goInception-plus/executor/mockAnalyzeSamplingBuildWorkerPanic"), IsNil)

	c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/executor/mockAnalyzeSamplingMergeWorkerPanic", "return(1)"), IsNil)
	err = tk.ExecToErr("analyze table t")
	c.Assert(err, NotNil)
	c.Assert(failpoint.Disable("gitee.com/zhoujin826/goInception-plus/executor/mockAnalyzeSamplingMergeWorkerPanic"), IsNil)
}
...
func (s *testSuite10) TestSnapshotAnalyze(c *C) {
	...
	tk.MustExec("commit")
	c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/executor/injectAnalyzeSnapshot", fmt.Sprintf("return(%d)", startTS1)), IsNil)
	tk.MustExec("analyze table t")
	...
	c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/executor/injectAnalyzeSnapshot", fmt.Sprintf("return(%d)", startTS2)), IsNil)
	tk.MustExec("analyze table t")
	rows = tk.MustQuery(fmt.Sprintf("select count, snapshot from mysql.stats_meta where table_id = %d", tid)).Rows()
	c.Assert(len(rows), Equals, 1)
