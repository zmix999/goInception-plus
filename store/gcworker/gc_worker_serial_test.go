...
func TestDeleteRangesFailure(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			require.NoError(t, failpoint.Enable("gitee.com/zhoujin826/goInception-plus/store/gcworker/mockHistoryJobForGC", "return(1)"))
			defer func() {
				require.NoError(t, failpoint.Disable("gitee.com/zhoujin826/goInception-plus/store/gcworker/mockHistoryJobForGC"))
			}()
...
func TestResolveLockRangeInfine(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("gitee.com/zhoujin826/goInception-plus/store/gcworker/setGcResolveMaxBackoff", "return(1)"))
	defer func() {
		require.NoError(t, failpoint.Disable("tikvclient/invalidCacheAndRetry"))
		require.NoError(t, failpoint.Disable("gitee.com/zhoujin826/goInception-plus/store/gcworker/setGcResolveMaxBackoff"))
	}()
...
func TestResolveLocksPhysical(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("gitee.com/zhoujin826/goInception-plus/store/gcworker/resolveLocksAcrossRegionsErr", "return(100)"))
	physicalUsed, err = s.gcWorker.resolveLocks(ctx, safePoint, 3, true)
	require.False(t, physicalUsed)
	require.NoError(t, err)
	require.NoError(t, failpoint.Disable("gitee.com/zhoujin826/goInception-plus/store/gcworker/resolveLocksAcrossRegionsErr"))
	...
	wg.Add(1)
	require.NoError(t, failpoint.Enable("gitee.com/zhoujin826/goInception-plus/store/gcworker/beforeCheckLockObservers", "pause"))
	go func() {
		defer wg.Done()
		physicalUsed, err := s.gcWorker.resolveLocks(ctx, safePoint, 3, true)
		require.True(t, physicalUsed)
	...
	require.NoError(t, failpoint.Disable("gitee.com/zhoujin826/goInception-plus/store/gcworker/beforeCheckLockObservers"))
	wg.Wait()
...
func TestPhysicalScanLockDeadlock(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("gitee.com/zhoujin826/goInception-plus/store/gcworker/resolveLocksAcrossRegionsErr", "return(1000)"))
	defer func() {
		require.NoError(t, failpoint.Disable("gitee.com/zhoujin826/goInception-plus/store/gcworker/resolveLocksAcrossRegionsErr"))
	}()
...
func TestGCLabelRules(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("gitee.com/zhoujin826/goInception-plus/store/gcworker/mockHistoryJob", "return(\"schema/d1/t1\")"))
	defer func() {
		require.NoError(t, failpoint.Disable("gitee.com/zhoujin826/goInception-plus/store/gcworker/mockHistoryJob"))
	}()
