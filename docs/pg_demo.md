...
sql = '''/*--user=root;--password=root;--host=127.0.0.1;--check=1;--port=5432;*/
...
conn = psycopg2.connect(host='127.0.0.1', user='', passwd='',
                       db='', port=5000, charset="")
cur = conn.cursor()
ret = cur.execute(sql)
result = cur.fetchall()
cur.close()
...
func main() {
    ...
    db, err := sql.Open("postgres", "host=127.0.0.1 port=5000 user=root password=root dbname=postgres sslmode=disable")
    defer db.Close()

    sql := `/*--user=test;--password=test;--host=127.0.0.1;--port=5432;--check=1;*/
    inception_magic_start;
    use  test;
    ...
    tbl.SetHeader(cols[0], cols[1], cols[2], cols[3], cols[4], cols[5], cols[6], cols[7], cols[8], cols[9], cols[10], cols[11], cols[12])
    ...
    for rows.Next() {
        ...
        tbl.AddRow(string(order_id), string(stage), string(error_level), string(stage_status), string(error_message), string(sql), string(affected_rows), string(sequence), string(backup_dbname), string(execute_time), string(sqlsha1), string(backup_time), string(needMerge))
