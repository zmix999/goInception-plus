...
import (
	...
	"github.com/zmix999/goInception-plus/server"
...
func (s *testCommon) initSetUp(c *C) {
	...
	testleak.BeforeTest()
	//s.cluster, _ = mock.NewCluster()
	//mockstore.BootstrapWithSingleStore(s.cluster)
	...
	s.session.SetProcessInfo("", time.Now(), mysql.ComQuery, 0)
...
func getLeftTable(node ast.ResultSetNode) *ast.TableSource {
	...
	case *ast.SetOprStmt:
		for _, sel := range x.SelectList.Selects {
			return getLeftTable(sel.(ast.ResultSetNode))
		}
	}
	return nil
}
...
func (s *testCommon) reset() {
	...
	logCfg.Level = "error"
	logCfg.Format = "text"
	logCfg.DisableTimestamp = false
	_ = logutil.InitLogger(&logCfg)
}
