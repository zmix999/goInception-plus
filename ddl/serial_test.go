...
func (s *testSerialSuite) TestCancelAddIndexPanic(c *C) {
	c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/ddl/errorMockPanic", `return(true)`), IsNil)
	defer func() {
		c.Assert(failpoint.Disable("gitee.com/zhoujin826/goInception-plus/ddl/errorMockPanic"), IsNil)
	}()
	tk := testkit.NewTestKit(c, s.store)
	tk.MustExec("use test")
	tk.MustExec("drop table if exists t")
...
func (s *testSerialSuite) TestRecoverTableByJobIDFail(c *C) {
	...
	hook.OnJobRunBeforeExported = func(job *model.Job) {
		if job.Type == model.ActionRecoverTable {
			...
			c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/ddl/mockRecoverTableCommitErr", `return(true)`), IsNil)
		}
	}
	origHook := s.dom.DDL().GetHook()
	defer s.dom.DDL().(ddl.DDLForTest).SetHook(origHook)
	...
	c.Assert(failpoint.Disable("gitee.com/zhoujin826/goInception-plus/ddl/mockRecoverTableCommitErr"), IsNil)
...
func (s *testSerialSuite) TestCancelJobByErrorCountLimit(c *C) {
	...
	defer func() {
		c.Assert(failpoint.Disable("gitee.com/zhoujin826/goInception-plus/ddl/mockExceedErrorLimit"), IsNil)
	}()
	tk.MustExec("use test")
	tk.MustExec("drop table if exists t")
...
func (s *testSerialSuite) TestTruncateTableUpdateSchemaVersionErr(c *C) {
	tk := testkit.NewTestKit(c, s.store)
	c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/ddl/mockTruncateTableUpdateVersionError", `return(true)`), IsNil)
	tk.MustExec("use test")
	tk.MustExec("drop table if exists t")
	...
	// Disable fail point.
	c.Assert(failpoint.Disable("gitee.com/zhoujin826/goInception-plus/ddl/mockTruncateTableUpdateVersionError"), IsNil)
...
func (s *testSerialDBSuite) TestCreateTableNoBlock(c *C) {
	tk := testkit.NewTestKitWithInit(c, s.store)
	c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/ddl/checkOwnerCheckAllVersionsWaitTime", `return(true)`), IsNil)
	defer func() {
		c.Assert(failpoint.Disable("gitee.com/zhoujin826/goInception-plus/ddl/checkOwnerCheckAllVersionsWaitTime"), IsNil)
	}()
