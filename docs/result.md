...
The information returned by goInception-plus is described as follows:
* When there is an error in the basic information submitted to goInception-plus, such as incomplete source information, a call exception will be returned, including error code and error information. The exception is the same as that of the MySQL server, and it can be handled in the same way as MySQL.
