...
use test;
select * from t;`
	c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/executor/errorMockParseSlowLogPanic", `return(true)`), IsNil)
	defer func() {
		c.Assert(failpoint.Disable("gitee.com/zhoujin826/goInception-plus/executor/errorMockParseSlowLogPanic"), IsNil)
	}()
