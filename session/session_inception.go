...
var (
	...
	skipSqlList = []string{"select @@version_comment limit 1",
		...
		"set names utf8", "set names utf8mb4", "set autocommit = 0", "begin"}
...
func (s *session) executeInc(ctx context.Context, sql string) (recordSets []sqlexec.RecordSet, err error) {
	...
	for i, sql_line := range sqlList {
		...
		if ((strings.HasSuffix(sql_line, ";") || strings.HasSuffix(sql_line, ";\r") && (strings.HasSuffix(sql_line, "DELIMITER;") || strings.HasSuffix(sql_line, "DELIMITER\r;"))) &&
			...
			stmtNodes, _, err := s.ParseSQL(ctx, s1)
	...
	case *ast.CreateIndexStmt:
		...
		if !s.inc.AlterAutoMerge || s.dbType == DBTypeOceanBase { // jwx added
			s.checkCreateIndex(node.Table, node.IndexName,
				node.IndexPartSpecifications, node.IndexOption, nil, Unique, tp)
		} else {
...
func (s *session) executeRemoteStatement(record *Record, isTran bool) {
	...
	if isTran {
		...
		res, err = s.execSQL(sqlStmt, false)
	}
	...
	for _, nc := range c.NewColumns {
		...
		if !found {
			...
			if nc.Options == nil {
				...
				for _, op := range nc.Options {
					defaultValue := fmt.Sprint(op.Expr.(ast.ValueExpr).GetValue())
	...
	if len(field.Options) > 0 {
		for _, op := range field.Options {
			...
			case ast.ColumnOptionComment:
				if op.Expr.(ast.ValueExpr).GetDatumString() != "" {
					hasComment = true
				}
			case ast.ColumnOptionNotNull:
				notNullFlag = true
	...
	if view != nil {
				...
				case *ast.SelectStmt:
					...
					if selectStmt.Fields != nil {
						...
						if hasWildCard {
							...
							selectColumnCount, err := s.subSelectColumns(sel.(ast.ResultSetNode))
		...
		if !s.hasError() {
			...
			if len(node.Cols) > 0 {
				...
				cols := s.getSubSelectColumns(node.Select.(ast.ResultSetNode))
				table.Fields = make([]FieldInfo, len(cols))
				for index, field := range cols {
					table.Fields[index] = FieldInfo{
						Field: field,
		...
		for i, list := range node.Lists {
			...
			for colIndex, vv := range list {
				...
				if v, ok := vv.(ast.ValueExpr); ok {
					...
					if v.GetType().Tp == mysql.TypeNull {
						...
						if tp, ok := columnsIsDate[name]; ok {
							_, err := GetTimeValue(s, v, tp, int8(v.GetType().Decimal))
							if err != nil {
		...
		if sel != nil {
			...
			if !s.hasError() {
				...
				if haveNewTable {
					...
					// 添加sql语句check，避免错误解析
					_, _, err := s.ParseSQL(context.Background(), selectSql)
					if err != nil {
						var builder strings.Builder
...
func (s *session) subSelectColumns(node ast.ResultSetNode) (int, error) {
	...
	case *ast.SetOprStmt:
		// 取第一个select的列数
		return s.subSelectColumns(sel.SelectList.Selects[0].(ast.ResultSetNode))
...
func (s *session) getSubSelectColumns(node ast.ResultSetNode) []string {
	...
	case *ast.SetOprStmt:
		// 取第一个select的列数
		return s.getSubSelectColumns(sel.SelectList.Selects[0].(ast.ResultSetNode))
	...
	for i := 0; i < values.NumField(); i++ {
		if values.Field(i).CanInterface() { //判断是否为可导出字段
			if k := t.Field(i).Tag.Get("toml"); strings.EqualFold(k, name) ||
				...
				err := s.setConfigValue(name, values.Field(i), value)
				if err != nil {
					return err
				}
				found = true
...
func (s *session) setConfigValue(name string, field reflect.Value, value ast.ValueExpr) (err error) {
...
func (s *session) executeLocalOscPause(node *ast.ShowOscStmt) ([]sqlexec.RecordSet, error) {
	...
	if pi, ok := pl[node.Sqlsha1]; ok {
		...
		if pi.SocketFile == "" {
			...
			cmd := EXecCmd.Command("sh", "-c", fmt.Sprintf("echo throttle | nc -U %s", pi.SocketFile))
	...
	if pi, ok := pl[node.Sqlsha1]; ok {
		...
		if pi.Pause {
			if pi.SocketFile == "" {
				...
				cmd := EXecCmd.Command("sh", "-c", fmt.Sprintf("echo no-throttle | nc -U %s", pi.SocketFile))
				if err := cmd.Run(); err != nil {
					err = fmt.Errorf("failed running command: %s %s; error=%v", "echo throttle | nc -U ", pi.SocketFile, err)
					log.Error(err)
				}
	...
	if ok1 && ok2 && val != nil {
		...
		if field != nil {
			...
			case "bit", "tinyint", "smallint", "mediumint", "int", "integer",
				"bigint", "decimal", "float", "double", "real":
				if !types.IsTypeNumeric(val.GetType().Tp) {
			...
			case "date", "time", "datetime", "timestamp",
				...
				// "year",
				// "geometry", "point", "linestring", "polygon",
				if !types.IsString(val.GetType().Tp) && !types.IsTypeTemporal(val.GetType().Tp) {
					s.appendErrorNo(ErrImplicitTypeConversion, tableName, field.Field, fieldType)
				}
			}
		}
...
func (s *session) checkItem(expr ast.ExprNode, tables []*TableInfo, cmmonTable string) bool {
	...
	case *ast.ExistsSubqueryExpr:
		s.checkSelectItem(e.Sel.(*ast.SubqueryExpr), tables, false, cmmonTable)
...
func (s *session) buildNewColumnToCache(t *TableInfo, field *ast.ColumnDef) *FieldInfo {
	...
	for _, op := range field.Options {
		switch op.Tp {
		case ast.ColumnOptionComment:
			c.Comment = op.Expr.(ast.ValueExpr).GetDatumString()
		case ast.ColumnOptionNull:
			c.Null = "YES"
