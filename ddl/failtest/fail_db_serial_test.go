...
import (
	...
	"gitee.com/zhoujin826/goInception-plus/ddl/testutil"
	ddlutil "gitee.com/zhoujin826/goInception-plus/ddl/util"
...
func TestHalfwayCancelOperations(t *testing.T) {
	...
	defer clean()
	require.NoError(t, failpoint.Enable("gitee.com/zhoujin826/goInception-plus/ddl/truncateTableErr", `return(true)`))
	defer func() {
		require.NoError(t, failpoint.Disable("gitee.com/zhoujin826/goInception-plus/ddl/truncateTableErr"))
	}()
	...
	require.NoError(t, failpoint.Enable("gitee.com/zhoujin826/goInception-plus/ddl/renameTableErr", `return("ty")`))
	defer func() {
		require.NoError(t, failpoint.Disable("gitee.com/zhoujin826/goInception-plus/ddl/renameTableErr"))
	}()
	...
	require.NoError(t, failpoint.Enable("gitee.com/zhoujin826/goInception-plus/ddl/exchangePartitionErr", `return(true)`))
	defer func() {
		require.NoError(t, failpoint.Disable("gitee.com/zhoujin826/goInception-plus/ddl/exchangePartitionErr"))
	}()
...
func TestInitializeOffsetAndState(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("gitee.com/zhoujin826/goInception-plus/ddl/uninitializedOffsetAndState", `return(true)`))
	tk.MustExec("ALTER TABLE t MODIFY COLUMN b int FIRST;")
	require.NoError(t, failpoint.Disable("gitee.com/zhoujin826/goInception-plus/ddl/uninitializedOffsetAndState"))
}

func TestUpdateHandleFailed(t *testing.T) {
	s, clean := createFailDBSuite(t)
	defer clean()
	require.NoError(t, failpoint.Enable("gitee.com/zhoujin826/goInception-plus/ddl/errorUpdateReorgHandle", `1*return`))
	defer func() {
		require.NoError(t, failpoint.Disable("gitee.com/zhoujin826/goInception-plus/ddl/errorUpdateReorgHandle"))
	}()
	tk := testkit.NewTestKit(t, s.store)
...
func TestAddIndexFailed(t *testing.T) {
	s, clean := createFailDBSuite(t)
	defer clean()
	require.NoError(t, failpoint.Enable("gitee.com/zhoujin826/goInception-plus/ddl/mockBackfillRunErr", `1*return`))
	defer func() {
		require.NoError(t, failpoint.Disable("gitee.com/zhoujin826/goInception-plus/ddl/mockBackfillRunErr"))
	}()
	tk := testkit.NewTestKit(t, s.store)
...
func TestGenGlobalIDFail(t *testing.T) {
	...
	defer clean()
	defer func() {
		require.NoError(t, failpoint.Disable("gitee.com/zhoujin826/goInception-plus/ddl/mockGenGlobalIDFail"))
	}()
	tk := testkit.NewTestKit(t, s.store)
	...
	for idx, test := range testcases {
		if test.mockErr {
			require.NoError(t, failpoint.Enable("gitee.com/zhoujin826/goInception-plus/ddl/mockGenGlobalIDFail", `return(true)`))
			_, err := tk.Exec(test.sql)
			require.Errorf(t, err, "the %dth test case '%s' fail", idx, test.sql)
		} else {
			require.NoError(t, failpoint.Enable("gitee.com/zhoujin826/goInception-plus/ddl/mockGenGlobalIDFail", `return(false)`))
...
func TestAddIndexWorkerNum(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			require.NoError(t, failpoint.Enable("gitee.com/zhoujin826/goInception-plus/ddl/checkBackfillWorkerNum", `return(true)`))
			defer func() {
				require.NoError(t, failpoint.Disable("gitee.com/zhoujin826/goInception-plus/ddl/checkBackfillWorkerNum"))
			}()
...
func TestRunDDLJobPanic(t *testing.T) {
	...
	defer clean()
	defer func() {
		require.NoError(t, failpoint.Disable("gitee.com/zhoujin826/goInception-plus/ddl/mockPanicInRunDDLJob"))
	}()
	tk := testkit.NewTestKit(t, s.store)
	tk.MustExec("use test")
	tk.MustExec("drop table if exists t")
	require.NoError(t, failpoint.Enable("gitee.com/zhoujin826/goInception-plus/ddl/mockPanicInRunDDLJob", `1*panic("panic test")`))
...
func TestPartitionAddIndexGC(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("gitee.com/zhoujin826/goInception-plus/ddl/mockUpdateCachedSafePoint", `return(true)`))
	defer func() {
		require.NoError(t, failpoint.Disable("gitee.com/zhoujin826/goInception-plus/ddl/mockUpdateCachedSafePoint"))
	}()
...
func TestPartitionAddPanic(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("gitee.com/zhoujin826/goInception-plus/ddl/CheckPartitionByRangeErr", `return(true)`))
	defer func() {
		require.NoError(t, failpoint.Disable("gitee.com/zhoujin826/goInception-plus/ddl/CheckPartitionByRangeErr"))
	}()
