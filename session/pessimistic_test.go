...
func (s *testPessimisticSuite) TestAsyncRollBackNoWait(c *C) {
	...
	c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/executor/ExecStmtGetTsError", "return"), IsNil)
	c.Assert(failpoint.Enable("tikvclient/beforeAsyncPessimisticRollback", "sleep(100)"), IsNil)
	defer func() {
		c.Assert(failpoint.Disable("gitee.com/zhoujin826/goInception-plus/executor/ExecStmtGetTsError"), IsNil)
