...
func (s *testEvaluatorSerialSuites) TestPushDownSwitcher(c *C) {
	...
	c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/expression/PushDownTestSwitcher", `return("all")`), IsNil)
	defer func() { c.Assert(failpoint.Disable("gitee.com/zhoujin826/goInception-plus/expression/PushDownTestSwitcher"), IsNil) }()
	...
	// All disabled
	c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/expression/PushDownTestSwitcher", `return("")`), IsNil)
	...
	c.Assert(failpoint.Enable("gitee.com/zhoujin826/goInception-plus/expression/PushDownTestSwitcher", fpexpr), IsNil)
	for i := range funcs {
		pbExpr := pc.ExprToPB(funcs[i])
		if !cases[i].enable {
			c.Assert(pbExpr, IsNil, Commentf("function: %s, sig: %v", cases[i].name, cases[i].sig))
